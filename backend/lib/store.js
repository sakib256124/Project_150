...
export function accountForPin(database, accountNumber, pin) { const normalized = String(accountNumber ?? '').trim().toUpperCase(); const account = database.accounts.find((item) => item.accountNumber === normalized); return account?.pin === String(pin) ? account : null; }
