...
export function renderLogin(container, onLogin) {
  ...
  container.innerHTML = `<section class="login-page"><div class="login-aside"><div class="brand"><span class="brand-mark">B</span> Bankly</div><div><p class="eyebrow" style="color:#c8e9ce">Bank management, made simple</p><h1>Clarity for every account.</h1><p>A practical management portal for accounts, deposits, transfers, bill payments, and transaction history.</p></div><div class="login-note">Educational demonstration project · Do not use real banking credentials.</div></div><div class="login-card-wrap"><div class="login-panel"><button class="theme-toggle login-theme-toggle" type="button" data-theme-toggle aria-pressed="${isDark}" aria-label="Switch to ${isDark ? 'light' : 'dark'} mode"><span class="theme-icon" data-theme-icon>${isDark ? '☀' : '◐'}</span><span data-theme-label>${isDark ? 'Light mode' : 'Dark mode'}</span></button><form id="login-form" class="login-card"><p class="eyebrow">Administrator portal</p><h2>Welcome back</h2><p class="subtle">Sign in to manage your bank records.</p><div class="demo-box">Demo access<br><strong>admin@bank.local</strong><br><strong>admin123</strong></div><label class="field">Email<input name="email" type="email" value="admin@bank.local" required></label><label class="field">Password<input name="password" type="password" value="admin123" required></label><button class="btn btn-primary btn-full">Sign in securely</button></form><p class="login-footer">© ${new Date().getFullYear()} Bankly · Educational workspace</p></div></div></section>`;
